...
assert.equal(ok('isChallengeDead', () => CH.isChallengeDead(null, null)), true)
assert.equal(ok('isChallengeExpired', () => CH.isChallengeExpired(null)), false)
assert.deepEqual(ok('expiredChallenges', () => CH.expiredChallenges(db)), [])
assert.deepEqual(ok('orphanedChallenges', () => CH.orphanedChallenges(db)), [])
assert.equal(ok('pendingStakeOf', () => CH.pendingStakeOf(db.challengePredictions, db.challenges, db.sessions, 'mid')), 0)
assert.equal(ok('availableSeasonPoints', () => CH.availableSeasonPoints(0, db.challengePredictions, db.challenges, db.sessions, 'mid')), 0)
assert.deepEqual(ok('settlePredictionsLocal', () => CH.settlePredictionsLocal(db.challengePredictions, 'c0', 'A', null)), [])
assert.equal(ok('canMemberPredict', () => CH.canMemberPredict(null, 'mid', db, 0)).ok, false)
