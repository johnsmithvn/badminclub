...
assert.equal(ok('isChallengeDead', () => CH.isChallengeDead(null)), true)
assert.equal(ok('pendingStakeOf', () => CH.pendingStakeOf(db.challengePredictions, db.challenges, 'mid')), 0)
assert.equal(ok('availableSeasonPoints', () => CH.availableSeasonPoints(0, db.challengePredictions, db.challenges, 'mid')), 0)
