...
export default function CourtAssignmentTab({ s }) {
  ...
  // State ghi điểm cho trận đang trên sân / kèo nhận nhanh
  ...
  const processedWaiting = useMemo(() => {
    ...
    if (sortOption === 'fewest') {
      list.sort((p1, p2) => (matchCountMap[p1.key] || 0) - (matchCountMap[p2.key] || 0) || compareVietnameseNames(p1.name, p2.name))
    } else if (sortOption === 'wait') {
      list.sort((p1, p2) => {
        ...
        return compareVietnameseNames(p1.name, p2.name)
      })
    } else if (sortOption === 'level') {
      list.sort((p1, p2) => (ratingsMap[p2.key] || 0) - (ratingsMap[p1.key] || 0) || compareVietnameseNames(p1.name, p2.name))
    } else {
      // Mặc định: az (tên từ A-Z)
      list.sort((p1, p2) => compareVietnameseNames(p1.name, p2.name))
    }
  ...
  const sortLabel = useMemo(() => {
    ...
    if (sortOption === 'fewest') return t('assign.sortFewest')
    return t('assign.sortAz')
  }, [sortOption])
  ...
  // 3. Mở modal ghi tỉ số cho trận đang đấu
  const handleOpenQuickScore = (om) => {
    setScoringMatch(om)
  }

  // 3b. Mở modal ghi tỉ số trực tiếp cho kèo đã nhận (1 chạm từ banner)
  ...
  // 4. Callback khi ScoreModal đã lưu kết quả thành công
  const handleScoreSaved = () => {
    if (scoringMatch && !scoringMatch.isChallengeDirectScore) {
  ...
  return (
    <div style={{ display: 'grid', gap: 14 }}>
          ...
          <div style={{ display: 'grid', gap: 10 }}>
            ...
            <div style={S.stickyBar}>
              ...
              <button
                ...
                style={{
                  ...S.sortChipMain,
                  ...(sortOption !== 'az' ? S.sortChipMainActive : {}),
                }}
              >
                {sortLabel} ▾
              </button>
      ...
      {/* Modal Ghi tỉ số cho trận đang trên sân / Kèo nhận nhanh */}
      {scoringMatch && (
        <ScoreModal
          court={scoringMatch}
          session={s}
          challenge={scoringMatch.selectedChallengeId ? (db.challenges || []).find((c) => c.id === scoringMatch.selectedChallengeId) : null}
          onClose={() => setScoringMatch(null)}
          onSaved={handleScoreSaved}
          onLoadToCourt={!scoringMatch.isChallengeDirectScore ? handleLoadOngoingToCourt : undefined}
        />
      )}
      ...
      <VoiceMatchModal
        ...
        onApplyResult={(res) => {
          if (voiceTargetMatch) {
            setScoringMatch({
              ...voiceTargetMatch,
              initialSets: [[res.scoreA, res.scoreB]],
            })
            setVoiceTargetMatch(null)
          } else {
