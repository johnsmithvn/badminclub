...
- [x] **Bóc tách số người đi ở banner Buổi đang mở & Chi tiết buổi (`Home.jsx: OpenNow`, `SessionDetail.jsx`, `money.js: openSessions`) — 2026-09-02**:
      - Tách riêng `fixedGoing / roster` (chỉ đếm thành viên cố định của ca), `extraGoing` (thành viên đi thêm), và `guests` (khách ngoài CLB).
      - Hiển thị cấu trúc rõ ràng: `[Tổng X người] · X/Y cố định · X đi thêm · X khách` ở Trang chủ và thẻ Điểm danh của Chi tiết buổi.
